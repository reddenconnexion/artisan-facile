-- Fix: restrict artisan profile fields returned by get_portal_data
-- Previously used row_to_json(p) which exposed sensitive fields:
-- email (personal login email), iban, ai_preferences, wero_phone, etc.
-- Now only returns public-facing contact fields.
...
BEGIN
  ...
  -- 3. Fetch Artisan Profile (only public-facing fields)
