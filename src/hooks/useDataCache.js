    ...
    return useQuery({
        ...
        queryFn: withOfflineCache(`dashboard_${user?.id}`, async () => {
            ...
            const { data: quotes, error: quotesError } = await supabase
                .from('quotes')
                .select('total_ht, total_ttc, date, created_at, status, id, client_id, clients(name), type, parent_id, signed_at, items, title, valid_until, quote_number, invoice_number, last_followup_at, follow_up_count, archived_at, relance_snoozed_until');
