-- Correctif : deux bugs dans sign_public_quote
--
-- Bug 1 : la colonne updated_at n'existe pas forcément dans quotes.
--         L'UPDATE échouait silencieusement (ou levait une exception) sans
--         que la fonction ne le détecte, et retournait quand même success:true.
--
-- Bug 2 : aucune vérification que l'UPDATE a bien modifié une ligne
--         (GET DIAGNOSTICS manquant). En cas d'UPDATE à 0 lignes la
--         fonction retournait success:true sans avoir rien enregistré.

-- 1. Ajouter updated_at à quotes si elle n'existe pas encore
ALTER TABLE public.quotes
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- Backfill pour les lignes existantes (valeur = created_at si présent, sinon NOW())
UPDATE public.quotes
SET updated_at = COALESCE(created_at, NOW())
WHERE updated_at IS NULL;

-- 2. Recréer sign_public_quote avec vérification GET DIAGNOSTICS
...
BEGIN
    ...
    -- Vérifier que l'UPDATE a bien modifié la ligne
