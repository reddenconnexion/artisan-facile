-- Acompte demandé directement sur le devis, exprimé en pourcentage du total TTC.
-- Permet d'inclure un acompte dès la création du devis (notamment pour les devis
-- 100 % main d'œuvre, où l'acompte matériel ne s'applique pas). 0 = pas d'acompte.
ALTER TABLE "public"."quotes"
  ADD COLUMN IF NOT EXISTS "deposit_percentage" numeric NOT NULL DEFAULT 0;

-- Réexpose tous les champs publics + le nouveau deposit_percentage, afin que le
-- PDF/portail consulté par le client affiche les conditions de règlement (acompte
-- à la signature / solde à la fin des travaux).
