...
const AmendmentFields = ({ formData, setFormData }) => {
    ...
    const parent = formData.parent_quote_data;
    const parentId = parent?.id || formData.parent_quote_id;
    const parentLabel = parent?.quote_number
        ? `DEV #${parent.quote_number}`
        : parentId
            ? `DEV #${parentId}`
            : null;
    ...
    return (
        <div className="space-y-0">
            {/* Header */}
            <div className="flex items-center justify-between gap-3 bg-orange-600 dark:bg-orange-700 px-5 py-3.5 rounded-t-xl">
                <div className="flex items-center gap-3 min-w-0">
                    <AlertTriangle className="w-5 h-5 text-white/90 flex-shrink-0" />
                    <h3 className="text-base font-semibold text-white tracking-wide">Détails de l'Avenant</h3>
                </div>
                {parentId && (
                    <Link
                        to={`/app/devis/${parentId}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        title={parent?.title ? `Ouvrir le devis parent : ${parent.title}` : 'Ouvrir le devis parent dans un nouvel onglet'}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/15 hover:bg-white/25 text-white text-xs font-medium transition-colors flex-shrink-0"
                    >
                        <FileText className="w-3.5 h-3.5" />
                        <span className="hidden sm:inline">Devis parent</span>
